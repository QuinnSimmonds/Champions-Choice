...
export default function HomePage() {
  ...
  const handleLogout = () => {
    logout();
    navigate("/"); // stay on home after logout
  };

  const displayName =
    user?.customerName || user?.vendorName || user?.username || "Account";
  ...
  return (
    <div style={{ backgroundColor: "#f5f5f5", minHeight: "100vh" }}>
      ...
      >
        <MDBContainer>
          ...
          <MDBTypography
            tag="p"
            className="lead mb-4"
            style={{ color: "#1565c0", fontSize: "1.3rem" }}
          >
      ...
      {/* ✅ NEW SURVEY CARD SECTION (Option 3) */}
      <MDBContainer className="my-5">
        <MDBCard className="p-4 shadow-3 text-center">
          <h4 className="fw-bold mb-3" style={{ color: "#0d47a1" }}>
            Get Personalized Recommendations!
          </h4>
          <p className="text-muted mb-4">
            Take a quick survey and help us recommend the best sports gear for you.
          </p>
          <Link to="/survey">
            <MDBBtn color="success" size="lg">
              Take Sports Survey
              <MDBIcon fas icon="clipboard-check" className="ms-2" />
            </MDBBtn>
          </Link>
        </MDBCard>
      </MDBContainer>
